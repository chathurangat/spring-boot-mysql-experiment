...
public class ScreenResponseGenerator {
    ...
    public HomeScreenResponse generate(List<Screen> screens, List<CategoryContent> categoryContents, Long screenId) {
        ...
        Map<Long, List<Content>> contentsMap = categoryContents.parallelStream().
                map(categoryContent ->
                        Content.builder()
                                ...
                                .images(categoryContent.getContent().getContentImages().parallelStream().map(
                                        contentImage ->
        ...
        List<Category> categories = activeScreen.getScreenCategories().parallelStream()
                .map(screenCategory -> Category.builder()
                        .id(screenCategory.getCategory().getId())
                        .name(screenCategory.getCategory().getName())
                        .sequenceNo(screenCategory.getSequenceNo()).contents(
                                contentsMap.get(screenCategory.getCategory().getId())
                        ).build())
                .collect(Collectors.toList());
        HomeScreenData homeScreenData1 = screenData.parallelStream().filter(homeScreenData -> homeScreenData.getId().equals(screenId)).findFirst().get();
